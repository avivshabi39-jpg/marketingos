...
export function StorageDashboard() {
  ...
  if (!data) return <div className="text-center py-8 text-slate-400 text-sm">טוען נתוני אחסון...</div>;
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5">
        ...
        <div className="flex justify-between text-sm mb-2">
          ...
          <span className="text-slate-500">מתוך {totals.neonFreeLimitMB} MB</span>
        </div>
        ...
        <p className="text-xs text-slate-400 text-left">{totals.usedPercent}%</p>
        ...
        <div className="flex gap-2 flex-wrap">
          {[5, 10, 20, 50].map((n) => (
            <div key={n} className="flex-1 min-w-[70px] bg-white rounded-lg p-2 text-center">
              <div className="text-[11px] text-slate-500">{n} לקוחות</div>
              <div className="text-sm font-extrabold text-green-700">₪{(n * 400).toLocaleString()}</div>
              <div className="text-[10px] text-slate-400">רווח: ₪{(n * 340).toLocaleString()}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5">
        ...
        <p className="text-xs text-slate-500 mb-3">מחק לידים לא-רלוונטיים כדי לפנות מקום</p>
        <div className="flex gap-2 mb-3 items-end">
          <div className="flex-1">
            <label className="text-[11px] font-semibold text-slate-600 block mb-1">ישנים מ:</label>
            <select value={daysOld} onChange={(e) => setDaysOld(Number(e.target.value))} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none">
          ...
          <button onClick={() => runCleanup(true)} disabled={cleaning} className="px-4 py-2 bg-slate-100 rounded-lg text-xs font-medium text-slate-700 disabled:opacity-50">
            {cleaning ? "..." : "🔍 בדוק"}
          </button>
          ...
          {clients.map((c) => (
            <div key={c.id} className="bg-slate-50 rounded-xl p-3">
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="text-xs text-slate-500">{c.storageKB} KB · {c.counts.leads} לידים</span>
              </div>
              <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min((c.storageKB / (totals.totalStorageKB || 1)) * 100, 100)}%` }} />
              </div>
            </div>
          ))}
        </div>
