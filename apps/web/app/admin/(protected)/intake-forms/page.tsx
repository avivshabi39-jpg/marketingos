  ...
  return (
    <div className="space-y-8" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">טפסי קבלה ולידים</h1>
          <p className="text-sm text-slate-500 mt-0.5">{clients.length} לקוחות פעילים</p>
        </div>
      ...
      {clients.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm flex flex-col items-center justify-center py-20">
          ...
          <p className="text-sm text-slate-500 mb-3">עדיין אין לקוחות פעילים</p>
          <Link href="/admin/clients/new" className="text-blue-500 hover:text-blue-700 text-sm font-medium">
            הוסף לקוח ראשון
          </Link>
        </div>
      )}
      ...
      {clients.length > 0 && (
        <section>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-base font-semibold text-slate-900">טפסי קבלת לקוח</h2>
              <p className="text-xs text-slate-500">לשימוש פנימי — ממלאים יחד עם לקוח חדש</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {clients.map((client) => {
              ...
              return (
                <div key={"ob-" + client.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3 mb-4">
                    <div
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-400">{count} תגובות</p>
                    </div>
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-50">
                    <Link
                      ...
                      className="flex items-center gap-1.5 text-sm font-medium text-blue-500 hover:text-blue-700 flex-1"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 border border-blue-200 rounded-lg px-2 py-1.5"
                    >
                      <FileText size={12} />
                      מלא טופס
                    </a>
      ...
      {clients.length > 0 && (
        <section>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-base font-semibold text-slate-900">טפסי לידים ללקוחות</h2>
              <p className="text-xs text-slate-500">הטפסים שהלקוחות שלך שמים בדפי הנחיתה שלהם</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {clients.map((client) => {
              ...
              return (
                <div key={"lp-" + client.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow">
                  ...
                  <div className="bg-slate-50 rounded-lg px-3 py-2 flex items-center gap-2 mb-3">
                    <span className="text-xs text-slate-500 truncate flex-1" dir="ltr">{intakeUrl}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-50">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-slate-700 flex-1"
                    >
                      צפה בלידים
                    </Link>
                    <a
