  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">הנכסים שלי</h1>
          <p className="text-slate-500 mt-0.5 text-sm">{properties.length} נכסים בסה&quot;כ</p>
        </div>
        <Link
          href={`/client/${params.slug}/properties/new`}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm whitespace-nowrap flex-shrink-0"
        >
          <Plus size={16} />
          הוסף נכס
        </Link>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        {properties.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-500 font-medium">אין נכסים עדיין</p>
            <p className="text-slate-400 text-sm mt-1 mb-5">הוסף את הנכס הראשון שלך כדי להתחיל</p>
            <Link
              href={`/client/${params.slug}/properties/new`}
                ...
                <thead className="bg-slate-50">
                  <tr>
                    {["נכס", "מחיר", "סטטוס", "סטטיסטיקות", "פעולות"].map((h) => (
                      <th
                        key={h}
                        className="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              {property.rooms != null && (
                                <span className="text-xs text-slate-400">{property.rooms} חד׳</span>
                              )}
                              {property.area != null && (
                                <span className="text-xs text-slate-400">{property.area} מ&quot;ר</span>
                              )}
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium whitespace-nowrap ${
                            STATUS_COLOR[property.status] ?? "bg-slate-100 text-slate-600"
                          }`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5 justify-end flex-wrap">
                          <Link
                            ...
                            className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors whitespace-nowrap"
                          >
                            <Edit size={12} />
                            עריכה
                          </Link>
              ...
              {properties.map((property) => (
                <div key={property.id} className="p-4">
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-50">
                    <Link
                      href={`/client/${params.slug}/properties/${property.id}/edit`}
                      className="flex-1 inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-md text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors"
                    >
                      <Edit size={12} />
                      עריכה
                    </Link>
